...
struct ServerInfo {
	...
	std::vector<std::string>ChatRoom;
} g_ServerInfo;
...
int Initialize() {	// Initialization 
	...
	printf("Creating theListen Socket . . . ");
	...
	for (;;) {
		...
		if (FD_ISSET(g_ServerInfo.listenSocket, &g_ServerInfo.socketsReadyForReading)) {
			...
			if (clientSocket == INVALID_SOCKET) {
				...
				printf("\nCommand Usage Instructions:\nEnter the ChatRoom Command :  [1][RoomName]\nLeave the ChatRoom Command : [2][RoomName]\nSend Message to the ChatRoom Command : [3][RoomName]\n");
				std::string Prompt;
				int sendResult = send(newClient.socket,Prompt.c_str(), Prompt.size() + 1, 0);

				if (sendResult == SOCKET_ERROR) {
					printf("Failure in Sending the Message(Error Found) %d\n", WSAGetLastError());
				}
				else {
		...
		for (int i = 0; i < g_ServerInfo.clients.size(); i++) {
			...
			if (FD_ISSET(client.socket, &g_ServerInfo.socketsReadyForReading)) {
				...
				if (recvResult == 0) {
					...
					printf("Failure in Receiving Message(Error Found) %d\n", WSAGetLastError());
				}
				else {
